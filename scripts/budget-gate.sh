...
echo "== css =="
for f in "$ASSETS"/*.css; do check "$f" 25 FAIL "css"; done
for f in "$ASSETS"/*.css; do check "$f" 15 WARN "css-warn"; done
echo "== src fonts referenced (S3: woff2 khong引用 = FAIL) =="
for f in apps/vanphat_portal/frontend/src/assets/fonts/*/*.woff2; do
  [ -f "$f" ] || continue
  base=$(basename "$f")
  if ! grep -rq "$base" apps/vanphat_portal/frontend/src apps/vanphat_portal/frontend/index.html 2>/dev/null; then
    echo "[FAIL] font-unreferenced: $f khong duoc tham chieu — xoa"
    FAIL=1
  else
    echo "[ok] font-referenced: $base"
  fi
done
