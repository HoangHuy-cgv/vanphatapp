...
| **Trục in ống đồng** | `Item` *(Nhóm Trục In)* | `TRUC-.#####` | `TRUC-00001` | Không reset | Mã quản lý tài sản trục in vật lý |
...
| `customer_name` | Tên pháp lý đầy đủ trên ĐKKD, dùng in hóa đơn VAT và hợp đồng | **Tên pháp nhân** |
| `alias` | Tên thương mại rút gọn, hiển thị trên UI buồng lái tác nghiệp | **Tên gọi tắt** |
...
| ERPNext Native English | Ý nghĩa nghiệp vụ | Thuật ngữ Tiếng Việt chuẩn |
| :--- | :--- | :--- |
| `item_code` | Mã định danh duy nhất quét tem barcode (`TP-`, `NGCS-`, `TMD-`, `TRUC-`, `NVL-`, `BTP-`) | **Mã sản phẩm** |
| `item_name` | Tên pháp lý đầy đủ (dùng xuất hóa đơn VAT, chứng từ kho, hợp đồng kinh tế) | **Tên pháp lý** |
| `custom_alias` *(Custom Field)* | Tên thương mại rút gọn hiển thị trên buồng lái (Cockpit UI / Bảng / Drawer) | **Tên gọi tắt (Alias UI)** |
| `description` | Mô tả kỹ thuật đầy đủ (kích thước, cấu trúc màng, độ dày, màu in, phụ kiện) | **Mô tả kỹ thuật** |
...
| `custom_accessory_spec` | Phụ kiện miệng túi (`Không`, `Vòi 16mm`, `Vòi 10mm`, `Khóa Zipper`) | **Phụ kiện miệng túi** |
...
| `alias` | Tên gọi tắt / tên thương mại hiển thị trên UI buồng lái & Đơn mua hàng | **Tên gọi tắt** |
