...
Stack: Vue 3.5 `<script setup>`, hash router (3 lazy routes: Orders/Quotes/Catalog, `keep-alive` read lists), frappe-ui (`FrappeUIProvider` once at root), Tailwind v3, Vite 7. Budget: entry gzip warn 170KB / fail 300KB (now 142KB).
...
- Depth in drawers: native `<dialog>` via `BaseDrawer` (Esc/backdrop close, focus restored). Center modals + confirm/prompt + toast from frappe-ui. No `tabindex` on `<dialog>`.
