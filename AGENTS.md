...
- **Forbidden Client-side Logic & Financial Math**: NEVER perform film math, pricing tiers, scrap rates, BOM derivations, VAT taxes, order totals, cylinder costs, or deposit calculations in `.vue`, `.js`, or `.ts`. Frontend MUST NEVER hardcode commercial/tax constants (e.g. `vatRate = 0.08`, `cylinderRate = 3100000`). 100% computations, pricing previews, and financial summaries MUST resolve via backend Python APIs (`vanphat_portal.api.*`) leveraging ERPNext native DocType methods.
- **Forbidden Client-side Partitioning & Tab Filtering**: Frontend MUST NOT fetch monolithic datasets and slice/filter by code prefixes (e.g. `startsWith('TP-')`, `startsWith('NVL-')`) or document stages via Javascript `.filter()`. 100% of tab filtering, category queries, and search lookups MUST pass parameters (`tab`, `category`, `status`, `search`) to backend Python APIs to let Frappe/MariaDB query cleanly.
...
- **API-First & Backend-Persistence Boundary**: 100% of CRUD operations and document workflow transitions (create, deposit, approve, submit, cancel) MUST resolve via whitelisted Python backend APIs (`vanphat_portal.api.*`) and persist directly to MariaDB before updating the UI. Order workflow APIs must reside in dedicated modules (`vanphat_portal.api.order`) instead of monolithic files.
