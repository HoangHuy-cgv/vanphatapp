...
export async function api(method, args = {}, options = {}) {
	try {
		...
		if (url.startsWith('http://') || url.startsWith('https://')) {
			// S7: chặn URL tuyệt đối ngoài origin — api() chỉ gọi ERP nội bộ,
			// không gắn CSRF header cho cross-origin.
			let sameOrigin = false;
			try {
				sameOrigin = new URL(url, window.location.origin).origin === window.location.origin;
			} catch (e) { sameOrigin = false; }
			if (!sameOrigin) {
				console.warn(`[api] blocked cross-origin url: ${method}`);
				if (!options.silent) toast.error('Yêu cầu không hợp lệ');
				return null;
			}
		}
		if (!url.startsWith('http://') && !url.startsWith('https://')) {
...
export async function boot() {
	// SPEC auth 2 mode (Sếp duyệt 2026-09-16): local build mode (vite dev)
	// không cần login. import.meta.env.DEV chỉ true khi `vite serve` — Vite
	// rớt hẳn nhánh này khỏi bundle prod (dead-code elimination), nên prod
	// không bao giờ chứa flag (triệt để hơn check window runtime).
	if (import.meta.env.DEV && typeof window !== 'undefined' && window.__VP_BUILD_MODE__) {
		currentUser.value = 'build-mode';
		return;
	}
	try {
